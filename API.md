...
| `ping` / `loss`                               | array              | 仅 `/api/servers` 的 `servers[]` 列表项返回，`/api/server` 详情接口不返回；后台开启三网详情时，从 D1 最近 1 小时历史按时间范围抽样并合并当前最新 Ping/丢包样本，最终最多 20 个点；D1 抽样结果在当前 Worker isolate 内缓存约 2 分钟。关闭三网详情时为空数组且不触发这部分 D1 查询。点格式为 `{ ts, ct, cu, cm, bd }`，`ct/cu/cm/bd` 分别对应电信、联通、移动、BGP |
